...
public class CellBroadcastSettings extends TimeConsumingPreferenceActivity
                ...
                if (errorInfo.equals("")) {
                    ...
                    if (!checkChannelIdExist(channelId)) {
                        ...
                        if (channel.getChannelState()) {
                            handleChannel(null, channel, objectList, INSERT_CHANNEL);
                        } else {
            ...
            public void onClick(DialogInterface dialog, int whichButton) {
                ...
                if (errorInfo.equals("")) {
                    ...
                    if (!checkChannelIdExist(newChannelId, oldChannel.getKeyId())) {
                        ...
                        handleChannel(oldChannel, newChannel, objectList, UPDATE_CHANNEL);
                    } else {
        ...
        if (channelBeginIndex != -1) {
            for (int j = channelBeginIndex; j <= channelEndIndex; j++) {
                ...
                if (channel != null) {
                    ...
                    String tName;
                    if  (getResources().getBoolean(R.bool.config_replace_default_cb_name)) {
                        if (50 == j) {
                            tName = getString(R.string.custome_cb_name);
                        } else {
                            //Modify DWYQFSQ-557 chenshu 20160810 start
                            //tName = getString(R.string.cb_default_new_channel_name) + jStr;
                            tName = getString(R.string.cb_default_new_channel_name);
                            //Modify DWYQFSQ-557 chenshu 20160810 end
                        }
                    } else {
                        //Modify DWYQFSQ-557 chenshu 20160810 start
                        //tName = getString(R.string.cb_default_new_channel_name) + jStr;
                    ...
                    //Add DWYQFSQ-501 chenshu 20160811 start------
                    if (!queryIfChannelInDatabase(mSubId, tName, j)) {
                    ...
                    mChannelArray.add(newChannel);
                    mChannelMap.put(jStr, newChannel);*/
                    //Add DWYQFSQ-501 chenshu 20160811 end-------
                }
            }
        }
    }
    ...
    private boolean queryIfChannelInDatabase(int mSubId,String channelName,int channelNum){
			MmsLog.d(LOG_TAG, "CellBroadcastSettings---queryIfChannelInDatabase: mSubId=" + mSubId
			...
			Cursor cursorzz = this.getContentResolver().query(Uri.parse("content://cb/channelnew"),
						projection,
						NUMBER + " = ? AND " +
						SUBID + " = ?",
			...
			if(cursorzz != null&&cursorzz.getCount() > 0){
				//if ((cursornum.getCount() != 0)&&(cursorid.getCount() != 0)){
					MmsLog.d(LOG_TAG, "queryIfChannelInDatabase: cursor.getCount"+cursorzz.getCount());
					cursorzz.close();
					return true;
					...
					cursorzz.close();
					MmsLog.d(LOG_TAG, "queryIfChannelInDatabase: return false");
					return false;
			}
    ...
    private void dumpConfigInfo(SmsBroadcastConfigInfo info) {
        MmsLog.d(LOG_TAG, "dump start for " + info.toString()
                + " FromServiceId " + info.getFromServiceId()
                + " ToServiceId " + info.getToServiceId()
                + " FromCodeId " + info.getFromCodeScheme()
                + " ToCodeId " + info.getToCodeScheme()
                + " dump end for " + info.toString());
    }
...
//            mAddChannelPreference.setEnabled(false);
                ...
                new DialogInterface.OnClickListener() {
                    public void onClick(DialogInterface dialog, int whichButton) {
                        ...
                        if (flag) {
                            ...
                            boolean isSetConfigSuccess = setCellBroadcastConfig(langList);
    ...
    private boolean setCellBroadcastConfig(SmsBroadcastConfigInfo[] objectList) {
    ...
    public boolean onContextItemSelected(MenuItem item) {
        ...
        case MENU_CHANNEL_ENABLE_DISABLE:
            ...
            if (!handleChannel(oldChannel, newChannel, objectList, UPDATE_CHANNEL)) {
                MmsLog.d(LOG_TAG, "oldChannel state = " + oldChannel.getChannelState());
                newChannel.setChannelState(!oldChannel.getChannelState());
            }
            break;
        ...
        case MENU_CHANNEL_DELETE:
            ...
            handleChannel(oldChannel, null, objectList1, DELETE_CHANNEL);
            break;
        default:
            break;
        }
    ...
    private boolean handleChannel(CellBroadcastChannel oldChannel, CellBroadcastChannel newChannel,
            SmsBroadcastConfigInfo[] objectList, int type) {
        MmsLog.d(LOG_TAG, "handleChannel oldChannel: " + oldChannel + " newChannel: " + newChannel
                + " type = " + type);
        if (!setCellBroadcastConfig(objectList)) {
            onError(mLanguagePreference, EXCEPTION_ERROR);
            return false;
        }
        switch (type) {
