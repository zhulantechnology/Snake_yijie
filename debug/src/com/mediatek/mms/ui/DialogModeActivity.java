...
import android.util.Log; //Add, JLLJ-110,JWLWKK-126 zhangyuwen, 20141105, Start
/** M:
 * Dialog mode
 */
public class DialogModeActivity extends Activity implements
    ...
    protected void onResume() {
        ...
        //Add, JLLJ-110,DWYBL-1356 JWLWKK-126 zhangyuwen, 20150423, Start
        updateSignature();
        //Add, JLLJ-110,DWYBL-1356 JWLWKK-126 zhangyuwen, 20150423, End
...
//        }
    ...
    private void resetMessage() {
        ...
        //Add, JLLJ-110, JWLWKK-126 zhangyuwen, 20141105, Start
        updateSignature();
        //Add, JLLJ-110, JWLWKK-126 zhangyuwen, 20141105, End
    ...
    private void updateCounter(CharSequence text, int start, int before, int count) {
        ...
        if (mReachMax == true) {
            ...
            if(MmsConfig.isEnable7BitSmsEncoding()) {
                SharedPreferences prefs = PreferenceManager.getDefaultSharedPreferences(DialogModeActivity.this);
                ...
                if(encondingMode != null && encondingMode.equals(SmsPreferenceActivity.SETTING_ENCONDING_MODE_7BIT)) {
                    mParams = SmsMessage.calculateLength(text, false, encodingType, encondingMode);
                }else{
                    mParams = SmsMessage.calculateLength(text, false, encodingType);
                }
            } else {
                mParams = SmsMessage.calculateLength(text, false);
            }
...
//            .calculateLength(text, false);
    ...
    private void updateSignature() {
    	if(mReplyEditor != null) {
    	    ...
    	    if(signature!=null) {
		int selection = mReplyEditor.getSelectionEnd();
		if(!mReplyEditor.getText().toString().endsWith(signature)) {
    		    mReplyEditor.append(signature);
    		    mReplyEditor.setSelection(selection);
		    //mWorkingMessage.setText(mReplyEditor.getText().toString()); //may had removed textChangedListener of mTextEditor
		} else {
		    int tmp = mReplyEditor.getText().length() - signature.length();
		    if(selection > tmp) {
			mReplyEditor.setSelection(tmp);
		    }
		}
    	    }
    	}
	...
	//Modify, DWYBL-647, DaiHongyi, 20150410, Start
	//if(isCurSMS() && mReplyEditor.getText().toString().trim().length() > 0) {
	if(mReplyEditor.getText().toString().trim().length() > 0) {
	//Modify, DWYBL-647, DaiHongyi, 20150410, End
	    Conversation conv = getConversation();
	    if(conv != null) {
		mWorkingMessage = WorkingMessage.createEmpty(this, this);
		...
		Log.e("TAG", "save draft error: conversation is null!");
	    }
	}
    }
    //Add, JLLJ-110,JWLWKK-126 zhangyuwen, 20141105, End
