...
import com.android.mms.transaction.MessagingNotification.NotificationProfile;
...
public class WapPushMessagingNotification {
    ...
    private static void playInConversationNotificationSound(Context context) {
        ...
        if (TextUtils.isEmpty(ringtoneStr)) {
            // Nothing to play
            MmsLog.w(TAG, "playInConversationNotificationSound ringtoneStr is empty");
            return;
        }
    ...
    private static void addWapPushNotificationInfos(
        ...
        try {
            while (cursor.moveToNext()) {
                ...
                MmsLog.d(TAG, "addWapPushNotificationInfos info = " + info);
    ...
    private static WapPushNotificationInfo getNewMessageNotificationInfo(
        ...
        MmsLog.d(TAG, "getNewMessageNotificationInfo clickIntent = " + clickIntent + "threadId = "
                + threadId + "uri = " + uri);
    ...
    private static void updateNotification(
        ...
        if (!NotificationPreferenceActivity.getNotificationEnabled(context)) {
            MmsLog.d(TAG, "updateNotification NotificationPreferenceActivity" +
                    ".getNotificationEnabled is false");
            return;
        }
        ...
        if (!OpMessageUtils.getOpMessagePlugin()
            ...
            noti.setContentIntent(
                    taskStackBuilder.getPendingIntent(0, PendingIntent.FLAG_UPDATE_CURRENT));
        }
        noti.setContentTitle(title).setPriority(Notification.PRIORITY_DEFAULT);
        ...
        if (isNew) {
            ...
            */
            ...
            if (audioManager.getVibrateSetting(AudioManager.VIBRATE_TYPE_NOTIFICATION)
                  ...
                  && notiProf.needVibrate()) {
                MmsLog.d(TAG, "updateNotification audioManager.getVibrateSetting " +
                        "== AudioManager.VIBRATE_SETTING_ON");
                defaults |= Notification.DEFAULT_VIBRATE;
            }

            String ringtoneStr = notiProf.needMute()
                    ? null : sp.getString(NotificationPreferenceActivity.NOTIFICATION_RINGTONE,
                    null);

        /// M: for Plug-in @{
            if (OpMessageUtils.
                    getOpMessagePlugin().getOpWappushMessagingNotificationExt() != null) {
                ringtoneStr = OpMessageUtils.getOpMessagePlugin()
                        .getOpWappushMessagingNotificationExt().getWapPushSimRingtone(
                        context, ringtoneStr, mostRecentNotification.mThreadId);
            }
        /// @}
            ...
            if (FeatureOption.MTK_BRAZIL_CUSTOMIZATION_CLARO) {
                ...
                if (state != TelephonyManager.CALL_STATE_IDLE) {
                    noti.setSound(TextUtils.isEmpty(ringtoneStr)
                            ? null : Uri.parse(ringtoneStr), AudioManager.STREAM_MUSIC);
                }
            } else {
    ...
    public static boolean notifySlAutoLanuchMessage(Context context, String url) {
        ...
        NotificationProfile notiProf = MessagingNotification.getNotificationProfile(context);
        ...
        NotificationManager nm = (NotificationManager) context.getSystemService(
                Context.NOTIFICATION_SERVICE);
        nm.notify(SL_AUTOLAUNCH_NOTIFICATION_ID, builder.build());
        return true;
    }

    private static NotificationProfile getNotificationProfile(Context context) {
        return MessagingNotification.getNotificationProfile(context);
    }
}
