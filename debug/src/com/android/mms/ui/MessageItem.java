...
public class MessageItem implements IMessageItemCallback {
    ...
    boolean mIsDrawScheduledDivider = false; //add JEYS-57 zhangzixiao 20131226
    ...
    MessageItem(Context context, int boxId, int messageType, int subId, int errorType, int locked,
        ...
        MmsLog.d(IPMSG_TAG, "MessageItem(): show time divider ?= "
                + mIsDrawTimeDivider + ", mTimeDividerString = "
            + mTimeDividerString);
        mIpMessageItem = IpMessageUtils.getIpMessagePlugin(context).getIpMessageItem();
...
//                                MessageUtils.formatTimeStampString(context, date));
            ...
            if(supportSyncSimMessage()) {
                indexOnIcc = cursor.getString(columnsMap.mColumnSmsIndexOnIcc);
            }
            //A: RGK-MMS-Feature: SyncSimMessages (END) }@
    ...
    //A: RGK-MMS-Feature: SyncSimMessages (START) @{
    public boolean isSimMessageInDb() {
		return supportSyncSimMessage() && indexOnIcc != null && !mSubMsg;
    }
	...
	return ScheduledMessageUtils.supportScheduledMessage(mContext) && mBoxId == 8;
    }
