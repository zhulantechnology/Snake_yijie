...
//Modify DSYYLM-50 chenshu 20160223 end
/**
...
//    private ConversationEmptyView mEmptyView;
    ...
    private static final int ENCRYPTION_NEW = 5;
    private static final int ENCRYPTION_GET = 6;
    ...
    protected void onCreate(Bundle savedInstanceState) {
		...
		if(getResources().getBoolean(R.bool.enable_mms_cs_ui)){
			setContentView(R.layout.conversation_list_screen2);
		} else {
			setContentView(R.layout.conversation_list_screen);
		}
		//Modify DSYYLM-50 chenshu 20160223 end
		...
		if(getResources().getBoolean(R.bool.enable_mms_cs_ui)){
			mImageButton = (ImageButton)findViewById(R.id.compose_button);
			//add jwlwel-297 chenshu 20150211 start
			//if(getResources().getBoolean(R.bool.default_menu_compose)){
			//	mImageButton.setVisibility(View.GONE);
			//}
			//add jwlwel-297 chenshu 20150211 end
		...
		if(getResources().getBoolean(R.bool.enable_mms_cs_ui)){
			...
			//-------------------------------------------------------------------
			mCustomSearchView = (SearchView)findViewById(R.id.search_owen);
		    mCustomSearchView.setOnQueryTextListener(mQueryTextListener);
        	mCustomSearchView.setIconifiedByDefault(true);		
			mCustomSearchView.onActionViewExpanded();
		    mCustomSearchView.setQueryHint(getString(R.string.search_hint));
		    mCustomSearchView.setFocusable(false);
		    mCustomSearchView.clearFocus();
		    SearchManager searchManager = (SearchManager) getSystemService(Context.SEARCH_SERVICE);

		    if (searchManager != null) {
		        SearchableInfo info = searchManager.getSearchableInfo(this.getComponentName());
		        mCustomSearchView.setSearchableInfo(info);
		    }
		  	try{
				Class<?> argClass = mCustomSearchView.getClass();
				Field mSearchViewEditTextField = argClass.getDeclaredField("mSearchSrcTextView");
				mSearchViewEditTextField.setAccessible(true);
				View mSearchViewEditText = (View)mSearchViewEditTextField.get(mCustomSearchView);
				mSearchViewEditText.setBackgroundDrawable(null);
		        Class<?> argClass1 = mCustomSearchView.getClass();
		        Field mSearchViewEditTextField1 = argClass1.getDeclaredField("mSearchSrcTextView");
		        mSearchViewEditTextField1.setAccessible(true);
		        AutoCompleteTextView mSearchViewEditText1 = (AutoCompleteTextView)mSearchViewEditTextField1.get(mCustomSearchView);
		        mSearchViewEditText1.setTextColor(0xff000000);
		    }catch(Exception e){
		    	e.printStackTrace();
		    }
		}
...
//        mIsJoynChanged = true;
    ...
    private void setupActionBar() {
		...
		ViewGroup v = null;
		if(getResources().getBoolean(R.bool.enable_mms_cs_ui)){
			v = (ViewGroup)LayoutInflater.from(this)
		...
		//Mofify no bug id chenshu 20160308 start
		...
		//Mofify no bug id chenshu 20160308 end

        mUnreadConvCount = (TextView) v.findViewById(R.id.unread_conv_count);
		if(!getResources().getBoolean(R.bool.enable_mms_cs_ui)){
			mImageButton = (ImageButton)findViewById(R.id.compose_button_new);
			mImageButton.setVisibility(View.VISIBLE);
			mImageButton.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
    ...
    private final ConversationListAdapter.OnContentChangedListener mContentChangedListener =
        ...
        public void onContentChanged(ConversationListAdapter adapter) {
            ...
            if (mIsInActivity) {
                ...
                if(mContext.getResources().getBoolean(R.bool.enable_mms_cs_ui)){
                    ((SlideCutListView)getListView()).resetSildeListViewStatus();
                }
            ...
            mSmsPromoBannerView.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View v) {
                    ...
                    if (!mOpConversationList.onClickSmsPromoBanner()) {
                        Intent settingIntent = new Intent(MmsApp.getApplication(),
    ...
    SearchView.OnQueryTextListener mQueryTextListener = new SearchView.OnQueryTextListener() {
        ...
        public boolean onQueryTextSubmit(String query) {
            ...
            intent.setClass(MmsApp.getApplication(), SearchActivity.class);
    ...
    public boolean onCreateOptionsMenu(Menu menu) {
		...
		if(getResources().getBoolean(R.bool.enable_mms_cs_ui)){
			getMenuInflater().inflate(R.menu.conversation_list_menu2, menu);
		} else {
			getMenuInflater().inflate(R.menu.conversation_list_menu, menu);
		}
		//Modify DSYYLM-50 chenshu 20160223 end
        ...
        if (FeatureOption.MTK_MWI_SUPPORT) {
			...
			//add DWQBBV-258 chenshu 20151123 start
            if (getResources().getBoolean(R.bool.never_show_mwi)) {
                mwiItem.setVisible(false);
            }
			//add DWQBBV-258 chenshu 20151123 end
        }
        /// @}
        return true;
    }
    ...
    public boolean onPrepareOptionsMenu(Menu menu) {
        ...
        /// @}
        //Add DWYSLM-116 chenshu 20160315 start unuse option so close it 
        item.setVisible(false);
        //Add DWYSLM-116 chenshu 20160315 end
        mIpConvList.onIpPrepareOptionsMenu(menu);
        item = menu.findItem(R.id.action_wappush);
		//Modify DWYSLM-116 chenshu 20160315 start
		if (getResources().getBoolean(R.bool.move_wappush_option)) {
			item.setVisible(false);
		} else {
			item.setVisible(true);
		}
        //item.setVisible(true);
		//Modify DWYSLM-116 chenshu 20160315 end
        ...
        //add DWQBBV-258 chenshu 20151123 start
        MenuItem itemSimMsg = menu.findItem(R.id.action_sim_msg);
        if (!getResources().getBoolean(R.bool.support_show_sim_message)) {
            itemSimMsg.setVisible(false);
        }
        //add DWQBBV-258 chenshu 20151123 end
		
		//add DWYQH-532 chengguangshuai 20170224 (start)
        MenuItem itemImportExport = menu.findItem(R.id.action_import_export);
        if (!MmsConfig.isSupportSmsImportExport()) {
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        switch(item.getItemId()) {
			...
			if (EncryptionActivity.supportEncryptedMessage()) {
				Cursor mCusor = this.getContentResolver()
						.query(Conversation.sAllThreadsUri,
								new String[] { "encrypted" }, "encrypted=1",
								null, null);
				if (EncryptionActivity.encryptionPreferenceEnabled()
						&& mCusor != null && mCusor.getCount() > 0) {
					Intent editEncryptionIntent = new Intent(
							MmsApp.getApplication(), EncryptionActivity.class);
					editEncryptionIntent.putExtra(EncryptionActivity.TYPE,
							EncryptionActivity.TYPE_DELETE);
					editEncryptionIntent.putExtra(
							EncryptionActivity.ALL_THREADS, 1);
					startActivityForResult(editEncryptionIntent,
							ENCRYPTION_DELETE);
				} else {
					confirmDeleteThread(-1L, mQueryHandler);
				}
			...
			case R.id.action_import_export:
                Intent backupRecoveryIntent = new Intent();
					backupRecoveryIntent.setClassName("com.mediatek.datatransfer", "com.mediatek.datatransfer.BootActivity");
                    backupRecoveryIntent.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
					try {
						startActivity(backupRecoveryIntent);
					} catch (Exception e) {
						Toast.makeText(this, R.string.str_ipmsg_no_app, Toast.LENGTH_LONG).show(); 
					}
                break;
            default:
                return true;
...
//                }
    ...
    public void onConfigurationChanged(Configuration newConfig) {
		...
		//Modify chenshu 20160310 start
		if (getResources().getBoolean(R.bool.enable_mms_cs_ui)) {
			if (newConfig.orientation == Configuration.ORIENTATION_LANDSCAPE){
				mCustomSearchView.setVisibility(View.GONE);
			} else {
...
//                }
        ...
        protected void onDeleteComplete(int token, Object cookie, int result) {
            ...
            case DELETE_CONVERSATION_TOKEN:
                ...
                //Modify DWYSLM-81 chenshu 20160512 start
                if(mContext.getResources().getBoolean(R.bool.enable_mms_cs_ui) && !isActionMode()){
                    ((SlideCutListView)getListView()).resetSildeListViewStatus();
                }
                //Modify DWYSLM-81 chenshu 20160512 end
    ...
    private class ModeCallback implements ActionMode.Callback {
        ...
        //Add chenshu DWYQLSSMY-64 20160405 start
        private MenuItem mDecryptItem;
        //Add chenshu DWYQLSSMY-64 20160405 end
        ...
        public boolean onCreateActionMode(ActionMode mode, Menu menu) {
            ...
            if (EncryptionActivity.supportEncryptedMessage()) {
                inflater.inflate(R.menu.conversation_multi_select_menu_with_selectall2, menu);
                mDecryptItem = menu.findItem(R.id.decrypt);
                if (!EncryptionActivity.encryptionPreferenceEnabled() || !EncryptionActivity.encryptionHasPassword()){
                    mDecryptItem.setShowAsAction(MenuItem.SHOW_AS_ACTION_NEVER);
                } else {
                    mDecryptItem.setShowAsAction(MenuItem.SHOW_AS_ACTION_ALWAYS);
                }
            } else {
                inflater.inflate(R.menu.conversation_multi_select_menu_with_selectall, menu);
            }
            ...
            //Modify DWYSLM-81 chenshu 20160512 start
            if(mContext.getResources().getBoolean(R.bool.enable_mms_cs_ui)){
                ((SlideCutListView)getListView()).setIsInLongClick(true);
            }
            //Modify DWYSLM-81 chenshu 20160512 end
            /// @}
        ...
        public boolean onActionItemClicked(final ActionMode mode, MenuItem item) {
            ...
            switch (item.getItemId()) {
				...
				// Modify DWYQYSB-232 chenshu 20160825 start
				/*
				 * if (mSelectedThreadIds.size() > 0) { Log.v(TAG,
				 * "ConversationList->ModeCallback: delete"); if
				 * (mDeleteAlertDialog != null &&
				 * mDeleteAlertDialog.isShowing()) { MmsLog.d(TAG,
				 * "no need to show delete dialog"); } else {
				...
				if (EncryptionActivity.supportEncryptedMessage()) {
					if (EncryptionActivity.encryptionPreferenceEnabled()) {
						if (mSelectedThreadIds != null
								&& mSelectedThreadIds.size() > 0) {
							String selection = null;
							android.util.Log.e("chenshu", "selectedThreads:"
									+ mSelectedThreadIds);
							StringBuilder buf = new StringBuilder();
							buf.append("encrypted=1 and ");
							buf.append("_id in ( ");
							int i = 0;
							long tempThread;
							for (Iterator it = mSelectedThreadIds.iterator(); it
									.hasNext();) {
								tempThread = (long) it.next();
								if (tempThread > 0) {
									android.util.Log.e("chenshu", "it.next():"
											+ tempThread);
									if (i++ > 0) {
										buf.append(",");
									}
							...
							android.util.Log.e("chenshu", "selection:"
									+ selection);
							int mCount = 0;
							Cursor mCusor = null;
							try {
								mCusor = ConversationList.this
										.getContentResolver().query(
												Conversation.sAllThreadsUri,
												new String[] { "encrypted" },
												selection, null, null);
								if (mCusor != null) {
									mCount = mCusor.getCount();
								}
							} catch (Exception e) {
							...
							if (EncryptionActivity
									.encryptionPreferenceEnabled()
									&& mCusor != null && mCount > 0) {
								android.util.Log.e("chenshu",
										"onActionItemClicked----> delete");
								Intent editEncryptionIntent = new Intent(
										MmsApp.getApplication(),
										EncryptionActivity.class);
								editEncryptionIntent.putExtra(
										EncryptionActivity.TYPE,
										EncryptionActivity.TYPE_DELETE);
								editEncryptionIntent.putExtra(
										EncryptionActivity.SELECTED_THREADS,
										mSelectedThreadIds);
								startActivityForResult(editEncryptionIntent,
										ENCRYPTION_DELETE);
								break;
								...
								Log.v(TAG,
										"ConversationList->ModeCallback: delete");
								if (mDeleteAlertDialog != null
										&& mDeleteAlertDialog.isShowing()) {
									MmsLog.d(TAG,
											"no need to show delete dialog");
								} else {
									confirmDeleteThreads(mSelectedThreadIds,
											mQueryHandler);
								}
							}
						} else {
							Log.v(TAG,
									"ConversationList->onActionItemClicked: delete--encryption--false");
			...
			// Modify DWYQYSB-232 chenshu 20160825 end
			    ...
			    //Modify chenshu 20151016 start no bug id, upgrade mms encryption start
                case R.id.encrypt:
                    android.util.Log.e("chenshu","onActionItemClicked---->encrypt");
                    if (EncryptionActivity.supportEncryptedMessage()) {
                        //first encrypt
                        if (mSelectedThreadIds != null && mSelectedThreadIds.size() > 0) {
                            if (!EncryptionActivity.encryptionPreferenceEnabled() && !EncryptionActivity.encryptionHasPassword()){
                                android.util.Log.e("chenshu","onActionItemClicked---->first encrypt");
                                Intent editEncryptionIntent = new Intent(MmsApp.getApplication(), EncryptionActivity.class);
                                editEncryptionIntent.putExtra(EncryptionActivity.TYPE, EncryptionActivity.TYPE_NEW);
                                editEncryptionIntent.putExtra(EncryptionActivity.SELECTED_THREADS,mSelectedThreadIds);
                                startActivityForResult(editEncryptionIntent, ENCRYPTION_NEW);
                                break;
                            } else {
                                android.util.Log.e("chenshu","onActionItemClicked---->continue encrypt");
                                for (Iterator it= mSelectedThreadIds.iterator(); it.hasNext();){
                            ...
                            //nothing to chose
                            Toast.makeText(ConversationList.this, R.string.tip_not_same_pwd, Toast.LENGTH_SHORT).show();
                        }
                    }
                    break;
                //Modify chenshu 20151016 start no bug id, upgrade mms encryption end
                //Modify chenshu 20160401 start no bug id, upgrade mms encryption start
                case R.id.decrypt:
                    android.util.Log.e("chenshu","onActionItemClicked---->decrypt");
                    if (EncryptionActivity.supportEncryptedMessage()) {
                       if (!EncryptionActivity.encryptionPreferenceEnabled() && !EncryptionActivity.encryptionHasPassword()){
                            android.util.Log.e("chenshu","onActionItemClicked---->first decrypt--nothing");
                            Toast.makeText(ConversationList.this, R.string.encrypt_first, Toast.LENGTH_LONG).show();
                        } else {
                            if (mSelectedThreadIds != null && mSelectedThreadIds.size() > 0) {
                                android.util.Log.e("chenshu","onActionItemClicked----> decrypt");
                                Intent editEncryptionIntent = new Intent(MmsApp.getApplication(), EncryptionActivity.class);
                                editEncryptionIntent.putExtra(EncryptionActivity.TYPE, EncryptionActivity.TYPE_GET);
                                editEncryptionIntent.putExtra(EncryptionActivity.SELECTED_THREADS,mSelectedThreadIds);
                                startActivityForResult(editEncryptionIntent, ENCRYPTION_GET);
                                break;
                            } else {
                                Toast.makeText(ConversationList.this, R.string.encrypt_first, Toast.LENGTH_LONG).show();
                            }
                    ...
                    break;
                //Modify chenshu 20160401 start no bug id, upgrade mms encryption end
                default:
                    break;
            }
            return true;
        ...
        public void onDestroyActionMode(ActionMode mode) {
            ...
            //Modify DWYSLM-81 chenshu 20160512 start
            if(mContext.getResources().getBoolean(R.bool.enable_mms_cs_ui)){
                ((SlideCutListView)getListView()).setIsInLongClick(false);
            }
            //Modify DWYSLM-81 chenshu 20160512 end
...
//            String selection = null;
    ...
    class ItemLongClickListener implements  ListView.OnItemLongClickListener {
        ...
        public boolean onItemLongClick(AdapterView<?> parent, View view, int position, long id) {
            //Modify DWYSLM-81 chenshu 20160512 start
            if(mContext.getResources().getBoolean(R.bool.enable_mms_cs_ui)){
                if (((SlideCutListView)getListView()).isDisableLongClick()){
                    return true;
                }
                ((SlideCutListView)getListView()).setIsInLongClick(true);
            }
            Log.d("SlideCutListView", "ItemLongClickListener return true");
            //Modify DWYSLM-81 chenshu 20160512 end
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (resultCode == RESULT_OK) {
            switch (requestCode) {
            case ENCRYPTION_NEW:
                android.util.Log.e("chenshu","RESULT_OK");
                Toast.makeText(ConversationList.this, R.string.pref_summary_enable_encrytpion, Toast.LENGTH_SHORT);
                break;
            case ENCRYPTION_GET:
                android.util.Log.e("chenshu","RESULT_OK--GET");
                Toast.makeText(ConversationList.this, R.string.menu_decryption, Toast.LENGTH_SHORT);
                break;
			case ENCRYPTION_DELETE:
				android.util.Log.e("chenshu", "RESULT_OK--DELETE");
				HashSet<Long> selectedThreads = (HashSet<Long>) data
						.getSerializableExtra(EncryptionActivity.SELECTED_THREADS);
				int allThreads = data.getIntExtra(
						EncryptionActivity.ALL_THREADS, -1);
				if (selectedThreads != null && selectedThreads.size() > 0) {
					Log.v(TAG, "ConversationList->onActivityResult: delete");
					if (mDeleteAlertDialog != null
							&& mDeleteAlertDialog.isShowing()) {
						MmsLog.d(TAG, "no need to show delete dialog");
					} else {
						if (selectedThreads.size() == 1) {
							Iterator it = selectedThreads.iterator();
							if (it.hasNext()) {
								confirmDeleteThread((long) it.next(),
										mQueryHandler);
							}
						} else {
							confirmDeleteThreads(selectedThreads, mQueryHandler);
						}
					}
...
//        }
    ...
    //Modify chenshu 20151016 start no bug id, upgrade mms encryption start
    private String getEncryptionValue() {
	    return PreferenceManager.getDefaultSharedPreferences(this).getString(KEY_ENCRYPTION, null);
    }

    private void setEncryptionPreferenceChecked(){
        PreferenceManager.getDefaultSharedPreferences(this).edit().putBoolean(ENABLE_ENCRYPTION, true).commit();
    }
    //Modify chenshu 20151016 start no bug id, upgrade mms encryption end
}
