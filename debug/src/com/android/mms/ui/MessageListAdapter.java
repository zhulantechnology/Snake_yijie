...
public class MessageListAdapter extends MessageCursorAdapter
    ...
    static {
	    if(MessageItem.supportSyncSimMessage()) {
    ...
    //A: RGK-MMS-Feature: SyncSimMessages (START) @{
    static final int COLUMN_SMS_INDEX_ON_ICC	= 32;
    //A: RGK-MMS-Feature: SyncSimMessages (END) }@
    ...
    public MessageListAdapter(
        ...
        mOpMessageListAdapterExt =
            OpMessageUtils.getOpMessagePlugin().getOpMessageListAdapterExt();
    }
                ...
                if (msgItem != null) {
                    ...
                    if (mIsDeleteMode) {
                        ...
                        if (msgItem.isSubMsg() && cursor.getColumnIndex("index_on_icc") != -1) {
                            ...
                            MmsLog.d(CACHE_TAG,
                                    "bindView(): type=" + type + ",simMsg msgIndex=" + msgIndex);
...
//                            }
    ...
    public MessageItem getCachedMessageItem(String type, long msgId, final Cursor c) {
        ...
        MmsLog.d(CACHE_TAG, "getCachedMessageItem(): key="
                + key + ", item is in cache?=" + (item != null));
        if (item == null && c != null && isCursorValid(c)) {
            ...
            if (type.equals("mms")) {
                ...
                final Runnable r = new Runnable() {
                    ...
                    public void run() {
                        MmsLog.d(CACHE_TAG,
                                "getCachedMessageItem(): call UI thread notify data set change.");
                        final Message msg =
                    ...
                    public void run() {
                        try {
                            ...
                            if (backgroundItem == null) {
                                ...
                                MmsLog.d(CACHE_TAG,
                                        "getCachedMessageItem(): put new" +
                                        " MessageItem into cache, messageId = -"
	    ...
	    if(ScheduledMessageUtils.supportScheduledMessage(mContext)) {
		item.mIsDrawScheduledDivider = isMessageItemShowScheduledDivider(c);
	    }
    ...
    //add JEYS-57 zhangzixiao 20131226 start
    private boolean isMessageItemShowScheduledDivider(Cursor cursor) {
	...
	if (cursor.isFirst()) {
	    return getItemViewType(cursor) == 8;
	}
        long currentBoxId = getItemBoxId(cursor);
        cursor.moveToPrevious();
        long previousBoxId = getItemBoxId(cursor);
        cursor.moveToNext();
        return (currentBoxId==8 && previousBoxId!=8);
    }
    private int getItemBoxId(Cursor cursor) {
        String type = cursor.getString(mColumnsMap.mColumnMsgType);
    ...
    public static class ColumnsMap implements IColumnsMapCallback {
        ...
        //A: RGK-MMS-Feature: SyncSimMessages (START) @{
        public int mColumnSmsIndexOnIcc;
        //A: RGK-MMS-Feature: SyncSimMessages (END) }@
        /// M: add for mms cc
        ...
        public ColumnsMap() {
            ...
            //A: RGK-MMS-Feature: SyncSimMessages (START) @{
            mColumnSmsIndexOnIcc      = COLUMN_SMS_INDEX_ON_ICC;
            //A: RGK-MMS-Feature: SyncSimMessages (END) }@
            /// M: add for mms cc
        ...
        public ColumnsMap(Cursor cursor) {
            ...
            try {
                mColumnMmsServiceCenter = cursor.getColumnIndexOrThrow(
                        Telephony.Mms.SERVICE_CENTER);
            } catch (IllegalArgumentException e) {
                MmsLog.e(TAG, e.getMessage());
            }
            ...
            try {
                if(MessageItem.supportSyncSimMessage()) {
                    mColumnSmsIndexOnIcc = cursor.getColumnIndexOrThrow(INDEX_ON_ICC);
                }
            } catch (IllegalArgumentException e) {
...
//                    }
    ...
    public void removeUselessItem(Cursor cursor) {
        if (cursor != null) {
            ...
            if (mMsgListItemHandler != null) {
                Message msg = mMsgListItemHandler.obtainMessage(
                        MultiDeleteActivity.UPDATE_SELECTED_COUNT);
    ...
    private boolean isMessageItemShowTimeDivider(Cursor cursor) {
        ...
        if (cursor.isFirst() || cursor.getCount() == 1) {
            MmsLog.w(TAG, "isMessageItemShowDivider(): cursor is the" +
                    " first or cursor count is 1! Is first?="
        ...
        MmsLog.w(TAG, "isMessageItemShowDivider(): currentMessageTime = "
                + sdf.format(new Date(currentMessageTime))
    ...
    private boolean isMessageItemShowOnlineDivider(Cursor cursor) {
        ...
        if (cursor.isBeforeFirst() || cursor.isAfterLast()) {
            MmsLog.w(TAG, "isMessageItemShowOnlineDivider():" +
                    " cursor is at before first or after last!");
            return false;
        }
