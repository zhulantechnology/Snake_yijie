...
public class SmsMessageSender implements MessageSender {
	...
	long scheduledTime = -1L; 
	boolean scheduled = ScheduledMessageUtils.supportScheduledMessage(mContext);
	if(scheduled) {
	    scheduledTime = ScheduledMessageUtils.getScheduledTime(mContext);
	    scheduled = scheduledTime > System.currentTimeMillis();
	}
	//add JEYS-57 zhangzixiao 20131226 end
        ...
        for (int i = 0; i < mNumberOfDests; i++) {
            try {
                ...
                if (smsUri == null) {
                    smsUri = Sms.addMessageToUri(mSubId,//add JEYS-57 20141212 "smsUri ="
		...
		//add JEYS-57 zhangzixiao 20131226 start
		if(scheduled) {
		    ScheduledMessageUtils.moveSmsToScheduled(mContext, smsUri, scheduledTime);
		    ScheduledMessageUtils.add(mContext, smsUri, scheduledTime);
		}
		//add JEYS-57 zhangzixiao 20131226 end
