...
public class TransactionService extends Service implements Observer, ITransactionServiceCallback {
    ...
    public void onNewIntent(Intent intent, int serviceId) {
        ...
        if (!PermissionCheckUtil.checkRequiredPermissions(this)) {
            MmsLog.i(MmsApp.TXN_TAG, "onNewIntent no permissions: serviceId: "
                        ...
                        switch (transactionType) {
                            ...
                            default:
                                ...
                                if (!SubscriptionManager.isValidSubscriptionId(subId)
                                        || (subId == 0)) {
                                    MmsLog.w(MmsApp.TXN_TAG, "onNewIntent invalid subId = "
                                            + subId);
                                } else {
    ...
    private void setTransactionById(Transaction transaction) {
        ...
        if (uri != null) {
            ...
            MmsLog.d(MmsApp.TXN_TAG, "----setTransaction() enter, uri: "
                    + uri + "  sProcessingTxn: " + sProcessingTxn);
        }
    }
    ...
    private String getTransactionId(Transaction transaction) {
        if (transaction == null) {
            MmsLog.e(MmsApp.TXN_TAG, "----getTransactionUri() transaction null ");
            return null;
        }
        String uri = transaction.getId();
        if (uri != null) {
            MmsLog.d(MmsApp.TXN_TAG, "----getTransactionUri() uri: "
    ...
    private void handleTransactionProcessed(Intent intent, int serviceId) {
        if (!mProcessing.isEmpty()) {
            if (intent.getData() == null) {
                MmsLog.e(MmsApp.TXN_TAG, "handleTransactionProcessed(), uri is null!");
                return;
            }
            ...
            if (transaction != null && transaction.getUri() != null) {
                ...
                if (transaction instanceof ReadRecTransaction) {
                    result = ((ReadRecTransaction) transaction).
                            checkSendResult(intent.getByteArrayExtra(SmsManager.EXTRA_MMS_DATA));
                }
                ...
                MmsLog.w(MmsApp.TXN_TAG, "handleTransactionProcessed(), uri not match!");
            }
        }
    }
    ...
    private void setTransactionFail(Transaction txn, int failType) {
        MmsLog.i(MmsApp.TXN_TAG, "set Transaction Fail. fail Type=" + failType);
    ...
    private boolean isDuringCall() {
        int[] subIds =
            SubscriptionManager.from(MmsApp.getApplication()).getActiveSubscriptionIdList();
    ...
    public void setCancelDownloadState(Uri uri, boolean isCancelling) {
        ...
        for (int i = 0; i < 2; i++) {
            synchronized (mProcessing) {
                for (Transaction t : mPending) {
                    ...
                    if (uriInList != null && (uriInList.equals(uri)
                        ...
                        if (isCancelling
                                && mOpTransactionServiceExt.cancelandRemoveTransaction()) {
                            mPending.remove(t);
                            cancelTransaction(t);
                            MmsLog.d(MmsApp.TXN_TAG,
                                    "***Cancel download when waiting connection!");
                        } else {
            ...
            /// M: If transacton not found both in mPending and mProcessing, wait and try again.
