...
import com.android.mms.ui.EncryptionActivity;//add chenshu JSLEL-1796 20141030
...
public class MessagingNotification {
	...
	if(supportDualRingtone()) {
	    MMS_STATUS_PROJECTION = new String[] {
    ...
    static {
	if(supportDualRingtone()) {
	    SMS_STATUS_PROJECTION = new String[] {
        	Sms.THREAD_ID, Sms.DATE, Sms.ADDRESS, Sms.SUBJECT, Sms.BODY, Sms._ID,
    ...
    //Modify, chenshu 20161118, start
    private static final int COLUMN_SMS_SIM_ID	= 7;
    ...
    public static final String IP_CHAT_DETAILS_BY_THREAD_ID = "content://chat_details_by_thread_id/"
    ...
    public static void blockingUpdateNewMessageIndicator(Context context, long newMsgThreadId,
        ...
        /// M: modify for RCSe
        setNotificationProfileByThreadId(context, newMsgThreadId, notiProf);

        boolean isContinuousComming = false;
        long currentTime = System.currentTimeMillis();
        if (newMsgThreadId != THREAD_NONE && currentTime >= sLastNotificationTime) {
            isContinuousComming = currentTime <= sLastNotificationTime + RINGTONE_WAIT_TIME;
            Log.d(TAG, "isContinuousComming = " + isContinuousComming +
                    " currentTime " + currentTime + "  " + sLastNotificationTime);
        }
        ...
        synchronized (sCurrentlyDisplayedThreadLock) {
            Log.d(TAG, "newMsgThreadId = " + newMsgThreadId +
            ...
            if (newMsgThreadId > 0 && newMsgThreadId == sCurrentlyDisplayedThreadId) {
                ...
                if (!isContinuousComming) {
                    notiProf.appRing = sOpMessagingNotification.blockingUpdateNewMessageIndicator(
                                             context, newMsgThreadId, notiProf.appRing,
                                             notiProf.appNotificationEnabled);
		      //Add, JWLW-1141, zhangzixiao, 20131118, Start
		      if(supportDualRingtone()) {
		           SortedSet<NotificationInfo> infos = new TreeSet<NotificationInfo>(INFO_COMPARATOR);
                    	    Set<Long> threads = new HashSet<Long>(4);
                    	    addMmsNotificationInfos(context, threads, infos);
                    	    addSmsNotificationInfos(context, threads, infos);
                    	    if(!infos.isEmpty()) {
                	        NotificationInfo info = infos.first();
                    	        int simId = info.simId;
                    	        notiProf.resetRingtoneStr(context, simId);
                    	     } else {
                	        Log.e("TAG", "playInConversationNotificationSound error : notificationInf set is empty!");
                    	     }
		      }
        ...
        if (notificationSet.isEmpty()) {
	     ...
	     //Modify, BWLLYL-479 chenshu 20161118, start
	     if(supportDualRingtone()) {
		   int simId = notificationSet.first().simId;
            	   notiProf.resetRingtoneStr(context, simId);
	     }
         //Modify, BWLLYL-479 chenshu 20161118, end
    ...
    private static void playInConversationNotificationSound(Context context,
        ...
        //modify chenshu 20161125 start
        if(supportDualRingtone()) {
            ringtoneStr = checkRingtone(context, ringtoneStr, notiProf.mSlotId);
        } else {
            ringtoneStr = checkRingtone(context, ringtoneStr);
        }
        //modify chenshu 20161125 end
    ...
    public static final class NotificationInfo {
        ...
        public CharSequence formatBigMessage(Context context) {
            ...
            if (mMessage != null) {
                ...
                spannableStringBuilder.append(IpMessageUtils.formatIpMessage(mMessage,
                        false, null));
            }
...
*/
        ...
        try {
            while (cursor.moveToNext()) {
                ...
                if (mmsThreads.size() > MAX_MESSAGES_TO_SHOW) {
                    ...
                    continue;
                } else if (mmsThreads.contains(threadId)
                        && addNotifyCount > MAX_MESSAGES_TO_SHOW) {
                ...
                //add BUG ID:JSLEL-1796 chenshu 20141030 start
                Conversation con = Conversation.get(context, threadId, false);
                if(EncryptionActivity.encryptionEnabled() && con != null && con.getEncryption() == 1) {
                    subject = "******";
                }
                //add BUG ID:JSLEL-1796 chenshu 20141030 end
                        ...
                        if (firstSlide != null) {
                            if (cursor.getCount() <= 1) {
                                if (firstSlide.hasImage()) {
                                    ...
                                    //Modify JSLEL-1795 chenshu 20141030 start            
                                    if(con != null && con.getEncryption() != 1) {
                                        attachedPicture = firstSlide.getImage().getBitmap(maxDim, maxDim);
                                    }
                                    //Modify JSLEL-1795 chenshu 20141030 end
                                }
                            }
                            if (firstSlide.hasText()) {
                            //Modify BUG ID:JSLEL-1796 chenshu 20141030 start
                                if(EncryptionActivity.encryptionEnabled() && con != null && con.getEncryption() == 1) {
                                    messageBody = "******";
                                }else  {
                                    messageBody = firstSlide.getText().getText();
                                }
                            //Modify BUG ID:JSLEL-1796 chenshu 20141030 end
                            }
                        }
                    }
			  ...
			  //Modify, BWLLYL-479 chenshu 20161118, start
			  if(supportDualRingtone()) {
			       int simId = cursor.getInt(COLUMN_MMS_SIM_ID);
	                     info.simId = simId;
			  }
	          //Modify,  BWLLYL-479 chenshu 20161118, end
                /// M:Code analyze 20,add a lock prevent multithreads
                /// accessing the same resources meantime
                /// avoid function sNotificationSet.add accessing
        ...
        try {
            while (cursor.moveToNext()) {
                ...
                if (smsThreads.size() > MAX_MESSAGES_TO_SHOW) {
                    ...
                    //threads count > 1, only need 1 item for every thread
                    continue;
                } else if (smsThreads.contains(threadId)
                        && addNotifyCount > MAX_MESSAGES_TO_SHOW) {
                    //only 1 thread, but has enough items for this thread
                    continue;
                }
                ...
                //add chenshu JSLEL-1796 20141030 start
                Conversation con = Conversation.get(context, threadId, false);
                if(EncryptionActivity.encryptionEnabled() && con != null && con.getEncryption() == 1) {
                    message = "******";
                }
                //add chenshu JSLEL-1796 20141030 end
                ...
                NotificationInfo info = getNewMessageNotificationInfo(context, isSms /* isSms */,
                        ...
                        Sms.CONTENT_URI.buildUpon()
                            .appendPath(Long.toString(cursor.getLong(COLUMN_SMS_ID))).build());
                /// @}
                //Modify, BWLLYL-479 chenshu 20161118, start
				  if(supportDualRingtone()) {
				       int simId = cursor.getInt(COLUMN_SMS_SIM_ID);
		                     info.simId = simId;
				  }
                //Modify, BWLLYL-479 chenshu 20161118, end
                /// M:Code analyze 20,add a lock prevent multithreads
    ...
    public static final NotificationInfo getNewMessageNotificationInfo(
        ...
        if (mIpMessagingNotification.onIpgetNewMessageNotificationInfo(number, threadId)) {
            clickIntent = new Intent(IP_CHAT_DETAILS_BY_THREAD_ID);
            clickIntent.putExtra("thread_id", threadId);
            clickIntent.putExtra("need_new_task", false);
        } else {
...
//                | Intent.FLAG_ACTIVITY_CLEAR_TOP);
    ...
    private static void updateNotification(
            ...
            int mmsUnReadCount,
            int smsUnReadCount) {
        Log.d(TAG, "isNew=" + isNew + "\tuniqueThreadCount="
                + uniqueThreadCount + "\tthreadId" + threadId);
        ...
        if (isNew && !isContinuousComming) {
            ...
            */
            ...
            //delete DWEQLSY-603 chengguangshuai 20161024 start
            //if (notiProf.needVibrate()
            //        && audioManager.shouldVibrate(AudioManager.VIBRATE_TYPE_NOTIFICATION)) {
            //    defaults |= Notification.DEFAULT_VIBRATE;
            //}
            //delete DWEQLSY-603 chengguangshuai 20161024 end
            /// @}
	        ...
	        if(supportDualRingtone()) {
				ringtoneStr = (isSystemNeedMute || notiProf.needMute()) ? null : notiProf.appRing;
			} else {
                ringtoneStr = sp.getString(NotificationPreferenceActivity.NOTIFICATION_RINGTONE,
                                                null);
				ringtoneStr = sOpMessagingNotification.updateNotification(context,
                                    mostRecentNotification.mUri, ringtoneStr);
            	ringtoneStr = (isSystemNeedMute || notiProf.needMute()) ? null : ringtoneStr;
			}
			/*ringtoneStr = sOpMessagingNotification.updateNotification(context,
                                    mostRecentNotification.mUri, ringtoneStr);
            ringtoneStr = (isSystemNeedMute || notiProf.needMute()) ? null : ringtoneStr;*/
			//Modify, BWLLYL-479 chenshu, 20161120, end
            //add DWEQLSY-603 chengguangshuai 20161024 start
            if (notiProf.needVibrate()
                    && (audioManager.shouldVibrate(AudioManager.VIBRATE_TYPE_NOTIFICATION) || ringtoneStr != null)) {
                defaults |= Notification.DEFAULT_VIBRATE;
            }
			//add DWEQLSY-603 chengguangshuai 20161024 end
            ...
            int state = ((TelephonyManager) context.
        ...
        if (messageCount == 1) {
            ...
            if (uniqueThreadCount == 1) {
                ...
                /// M:Code analyze 20,add a lock prevent multithreads
                /// accessing the same resources meantime,
                /// because the paragraph below is in multithreads branch,
                /// more likely to be accessed by more
    ...
    //Add, BWLLYL-479 chenshu, 20131118, Start
    public static void notifySendFailed(Context context, boolean noisy, int simId) {
	int slotId = SubscriptionManager.getSlotId(simId);
	notifyFailed(context, false, 0, noisy, slotId);
    }

    private static void notifyFailed(Context context, boolean isDownload, long threadId,
                                     boolean noisy) {
	notifyFailed(context, isDownload, threadId, noisy, 0);
    }
    //Add, BWLLYL-479 chenshu, 20131118, end

    //Add, BWLLYL-479 chenshu, 20131118, Start
    /*private static void notifyFailed(Context context, boolean isDownload, long threadId,
                                     boolean noisy) {*/
    private static void notifyFailed(Context context, boolean isDownload, long threadId,
                                     boolean noisy, int slotId) {
    //Add, BWLLYL-479 chenshu, 20131118, end
        ...
        if (noisy) {
	    	...
	    	if(supportDualRingtone()) {
				Uri uri;
                if(slotId == 1) {
					 uri = RingtoneManager.getActualDefaultMessageRingtoneUri(context, RingtoneManager.TYPE_MESSAGE_RINGTONE,1);
		    	     ringtoneStr = (uri != null? uri.toString() : null);
	    	    } else {
		    	     uri = RingtoneManager.getActualDefaultMessageRingtoneUri(context, RingtoneManager.TYPE_MESSAGE_RINGTONE,0);
		    	     ringtoneStr = (uri != null? uri.toString() : null);
	            }
            } else {
                ringtoneStr = (isSystemNeedMute || notiProf.needMute()) ? null : (sp.getString(
                    NotificationPreferenceActivity.NOTIFICATION_RINGTONE, null));
			}
			//Modify, BWLLYL-479 chenshu, 20161120, end	
            //add DWEQLSY-603 chengguangshuai 20161024 start
			...
			//add DWEQLSY-603 chengguangshuai 20161024 end
            ringtoneStr = sOpMessagingNotification.notifyFailed(context, threadId, ringtoneStr);
            ...
            processNotificationSound(context);
			//delete DWEQLSY-603 chengguangshuai 20161024 start
            //int defaults = 0;
            //if (notiProf.needVibrate() &&
            //        audioManager.shouldVibrate(AudioManager.VIBRATE_TYPE_NOTIFICATION)) {
        ...
        if (isDownload) {
            notificationMgr.notify(DOWNLOAD_FAILED_NOTIFICATION_ID, builder.build());
        } else {
            notificationMgr.notify(MESSAGE_FAILED_NOTIFICATION_ID, builder.build());
        }
    }
...
//        }
	...
	return notifyClassZeroMessage(context, address, 0);
    }
    //Add, JWLW-1141, zhangzixiao, 20131118, End
    ...
    //public static boolean notifyClassZeroMessage(Context context, String address) {
    public static boolean notifyClassZeroMessage(Context context, String address, int slotId) {
    //Modify, JWLW-1141, zhangzixiao, 20131118, End
		...
		//Modify, chenshu, 20161120, Start
        /*String ringtoneStr =
            sp.getString(NotificationPreferenceActivity.NOTIFICATION_RINGTONE, null);*/
			String ringtoneStr = null;
	    	if(supportDualRingtone()) {
				Uri uri;
                if(slotId == 1) {
                ...
                ringtoneStr = sp.getString(NotificationPreferenceActivity.NOTIFICATION_RINGTONE, null);
			}
			//Modify, chenshu, 20161120, end
        //modify chenshu 20161125 start
        if(supportDualRingtone()) {
            ringtoneStr = MessagingNotification.checkRingtone(context, ringtoneStr, slotId);
        } else {
        ...
        Uri ringtone = TextUtils.isEmpty(ringtoneStr) ? null : Uri.parse(ringtoneStr);
        processNotificationSound(context);
        builder.setTicker(address);
        int defaults = 0;
        NotificationProfile notiProf = getNotificationProfile(context);
        AudioManager audioManager = (AudioManager) context.getSystemService(Context.AUDIO_SERVICE);
		//modify DWEQLSY-603 chengguangshuai 20161024 start
        boolean isSystemNeedMute = false;
        // change for L1 API change, use getRingerModeInternal.
        ...
        isSystemNeedMute = isVibrateMode || isSilentMode || notiProf.needMute();
        if (notiProf.needVibrate() &&
                (audioManager.shouldVibrate(AudioManager.VIBRATE_TYPE_NOTIFICATION) || !isSystemNeedMute)) {
            defaults |= Notification.DEFAULT_VIBRATE;
        }
		//if (notiProf.needVibrate() &&
        //        audioManager.shouldVibrate(AudioManager.VIBRATE_TYPE_NOTIFICATION)) {
    ...
    public static void processNotificationSound(
		...
		boolean isSilentMode = audioManager.getRingerModeInternal() == AudioManager.RINGER_MODE_SILENT;
		isSystemNeedMute = isVibrateMode || isSilentMode || notiProf.needMute();
        if (notiProf.needVibrate() && (audioManager.shouldVibrate(AudioManager.VIBRATE_TYPE_NOTIFICATION) || !isSystemNeedMute)) {
    ...
    /// M:Code analyze 019,new method add for setting the ringtone of notification @{
    public static void processNotificationSound(Context context) {
        int state = ((TelephonyManager) context.getSystemService(
                Context.TELEPHONY_SERVICE)).getCallState();
        if (FeatureOption.MTK_BRAZIL_CUSTOMIZATION_CLARO
                && state != TelephonyManager.CALL_STATE_IDLE) {
    ...
    public static final class NotificationProfile {
	 ...
	 //Add, JWLW-1141, zhangzixiao, 20131118, End
        ...
        String getRingtoneStr() {
            return TextUtils.isEmpty(threadRing)
            ? (TextUtils.isEmpty(appRing) ? "" : appRing) : threadRing;
		}
	 //Add, JWLW-1141, zhangzixiao, 20131118, Start
        void resetRingtoneStr(Context context, int simId) {
		int slotId = SubscriptionManager.getSlotId(simId);
        mSlotId = slotId;
        	if(slotId == 1) {
        		appRing = appRing2;
        		//threadRing = threadRing2;
        	}
        }
        //Add, JWLW-1141, zhangzixiao, 20131118, End
    }
    ...
    public static NotificationProfile getNotificationProfile(Context context) {
        ...
        String muteStr =
            prefs.getString(NotificationPreferenceActivity.NOTIFICATION_MUTE, Integer.toString(0));
        ...
        //np.appRing = prefs.getString(NotificationPreferenceActivity.NOTIFICATION_RINGTONE, "");
          np.appRing = prefs.getString(NotificationPreferenceActivity.NOTIFICATION_RINGTONE, NotificationPreferenceActivity.DEFAULT_RINGTONE);
	    //Modify, JWLW-1141, zhangzixiao, 20131118, End
	     //Add, JWLW-1141, zhangzixiao, 20131118, Start
	     if(supportDualRingtone()) {
		      Uri appRing0 = RingtoneManager.getActualDefaultMessageRingtoneUri(context, RingtoneManager.TYPE_MESSAGE_RINGTONE, 0);
		      Uri appRing1 = RingtoneManager.getActualDefaultMessageRingtoneUri(context, RingtoneManager.TYPE_MESSAGE_RINGTONE, 1);
			  np.appRing =  appRing0 != null ? appRing0.toString() : null;
	          np.appRing2 = appRing1 != null ? appRing1.toString() : null;
	     }
	     //Add, JWLW-1141, zhangzixiao, 20131118, End
        //Modify, JWLW-1141, zhangzixiao, 20131118, Start
        ...
        if (np.appMuteStart > 0 && np.appMute > 0) {
            ...
            if ((np.appMute * 3600 + np.appMuteStart / 1000) <= currentTime) {
                ...
                editor.putString(NotificationPreferenceActivity.NOTIFICATION_MUTE,
                        String.valueOf(np.appMute));
                editor.apply();
            }
        }
    ...
    /**
     * Set notification profile for special thread.
     * @param context The context
     * @param threadId thread id of new message
     * @return NotificationProfile
     */
    public static void setNotificationProfileByThreadId(Context context,
            long threadId, NotificationProfile np) {
        MmsLog.d(TAG, "setNotificationProfileByThreadId: " + threadId);
        if (threadId > 0) {
            ...
            try {
                if (c.getCount() == 0) {
                    ...
                    MmsLog.d(TAG, "setNotificationProfileByThreadId: threadNotificationEnabled = "
                            + np.threadNotificationEnabled
        ...
        if (threadId > 0 && !np.threadNotificationEnabled) {
            MmsLog.d(TAG, "thread notification set disabled!" + threadId);
        }
    }
    ...
    private static void wakeUpScreen(Context context) {
        ...
        //Delete DSYYLM-52 chenshu 20160111 start
        //if (hasInsertedHeadSet || headsetIsOn) {
        //Delete DSYYLM-52 chenshu 20160111 end
        ...
        try {
            ...
            /*if (isMuteOrVibrate(context)) {
                ...
                if (sMuteModeToneGenerator != null) {
                    sMuteModeToneGenerator.startTone(ToneGenerator.TONE_SUP_DIAL,
                            MUTE_TONE_LENGTH_MS);
                }
            }*/
        //Delete DSYYLM-52 chenshu 20160111 start
    ...
    private static boolean isMuteOrVibrate(Context context) {
        ...
        MmsLog.d(TAG, "Audio manager ringer mode = " + audioManager.getRingerMode()
                + " stream volume = " + audioManager.getStreamVolume(AudioManager.STREAM_RING));
    ...
    public static final String checkRingtone(Context context, String ringtoneUri) {
        ...
        if (!TextUtils.isEmpty(ringtoneUri)
            ...
            if (invalidRingtone) {
				...
				if(supportDualRingtone()) {
					ringtoneUri = Settings.System.getString(context.getContentResolver(), RingtoneManager.KEY_DEFAULT_RINGTONE0);
				} else {
					ringtoneUri = NotificationPreferenceActivity.DEFAULT_RINGTONE;
				}
				//modify chenshu 20161120 end
        ...
        if (!TextUtils.isEmpty(ringtoneUri)
            ...
            if (uri != null) {
                if (RingtoneManager.isRingtoneExist(context, uri)) {
					...
					//Modify DWYQLSSMY-57 chenshu 20160328 start
					...
					//Modify DWYQLSSMY-57 chenshu 20160328 end
                }
            }
        }
    ...
    //add chenshu 20161125 start
    public static final String checkRingtone(Context context, String ringtoneUri, int mSlot) {
        Log.d(TAG, "checkRingtone ringtoneUri" + ringtoneUri);
        if (!TextUtils.isEmpty(ringtoneUri)
                && !ringtoneUri.equals(NotificationPreferenceActivity.DEFAULT_RINGTONE)) {
            InputStream inputStream = null;
            ...
            if (invalidRingtone) {
				...
				if(supportDualRingtone()) {
                    if (mSlot==0) {
                            ...
                            ringtoneUri = Settings.System.getString(context.getContentResolver(), RingtoneManager.KEY_DEFAULT_RINGTONE1);
                    } else {
                            ringtoneUri = Settings.System.getString(context.getContentResolver(), RingtoneManager.KEY_DEFAULT_RINGTONE0);
                    }
				} else {
    ...
    public static void showWfcNotification(Context context) {
        ...
        if (doShowWfcPopup(SubscriptionManager.INVALID_SUBSCRIPTION_ID, context)) {
    ...
    public static boolean doShowWfcPopup(int selectedSubId, Context context) {
        ...
        if (!MessageUtils.isWifiConnected(context)) {
            if (ImsManager.getWfcMode(context)
                    == ImsConfig.WfcModeFeatureValueConstants.WIFI_ONLY) {
                MmsLog.d(TAG, "Wifi-only & Wifi off or not connected:show pop-up");
                return true;
            }
            if (MessageUtils.getCellularState(selectedSubId, context) !=
