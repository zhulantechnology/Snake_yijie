...
public class MmsMessageSender implements MessageSender {
	...
	if(ScheduledMessageUtils.supportScheduledMessage(mContext)) {
	    long scheduledTime = ScheduledMessageUtils.getScheduledTime(mContext);
	    boolean scheduled = scheduledTime > System.currentTimeMillis();
	    if(scheduled) {
	    	Uri mmsUri = ContentUris.withAppendedId(Mms.CONTENT_URI, messageId);
	    	ScheduledMessageUtils.moveMmsToScheduled(mContext, mmsUri, scheduledTime);
	    	ScheduledMessageUtils.add(mContext, mmsUri, scheduledTime);
	    	ScheduledMessageUtils.resetScheduledTime(mContext);
	    	return true;
	    }   
	}
    ...
    public static void sendReadRec(Context context, String to,
        ...
        try {
            ...
            Uri uri = PduPersister.getPduPersister(context).persist(readRec,
