            ...
            try {
                ...
                switch (type) {
                    ...
                    case MESSAGE_TYPE_NOTIFICATION_IND: {
                        ...
                        mIpSpamMsgReceiver =
                            IpMessageUtils.getIpMessagePlugin(mContext).getIpSpamMsgReceiver();
				...
				String address = nInd.getFrom().getString();
        		    	long blacktextId = BlackMessageUtils.getNumberInBlacklist(mContext, address);
			   	Log.e(TAG, "handleMmsReceived, address="+address+",blacktextId="+blacktextId);
        		    	if(blacktextId > 0) {
				    long simId = -1l;
					/*
				    EncapsulatedSimInfoManager si = null;
				    if (EncapsulatedFeatureOption.MTK_GEMINI_SUPPORT) {
				    	si = EncapsulatedSimInfoManager.getSimInfoBySlot(mContext, 
						intent.getIntExtra(BlackMessageUtils.GEMINI_SIM_ID_KEY, -1));
				    } else {
				    	si = si = EncapsulatedSimInfoManager.getSimInfoBySlot(mContext, 0);
				    }
				    if(null != si) {
				    	simId = si.getSimId();
				    }
				    */
				    //zyw
				    ///SubInfoRecord mSubInfoRecord = SubscriptionManager.getSubInfoForSubscriber((long)intent.getIntExtra(BlackMessageUtils.GEMINI_SIM_ID_KEY, -1));
                                    //SubscriptionInfo mSubInfoRecord  = SubscriptionManager.from(MmsApp.getApplication())
                                           //.getActiveSubscriptionInfo(intent.getIntExtra(BlackMessageUtils.GEMINI_SIM_ID_KEY, -1));
                    SubscriptionInfo mSubInfoRecord  = SubscriptionManager.from(MmsApp.getApplication())
                                           .getActiveSubscriptionInfo(intent.getIntExtra(PhoneConstants.SUBSCRIPTION_KEY,
                    SubscriptionManager.INVALID_SUBSCRIPTION_ID));
			        Log.v(TAG, "mSubInfoRecord = " + mSubInfoRecord + ",mSubInfoRecord.slotId = " + mSubInfoRecord.getSimSlotIndex() );
			        if(mSubInfoRecord != null) {
						simId = mSubInfoRecord.getSimSlotIndex();
			            
				    }

				
				    String serviceAddr = intent.getStringExtra(WapPush.SERVICE_ADDR);
            			    BlackMessageUtils.insertMmsToBlacklist(mContext, nInd, blacktextId, simId, serviceAddr);
            			    break;
        		    	}
			    }
        		    //add JBLW-482 zhangzixiao 20131216 end
                        ...
                        if (pendingCursor != null) {
                            try {
                                if (pendingCursor.getCount() == 1 && pendingCursor.moveToFirst()) {
                                    ...
                                    MmsLog.w(MmsApp.TXN_TAG, "can not find message to "
                                            + " set pending sim id, msgId="
                                            + msgId);
                                }
                            } finally {
                                pendingCursor.close();
            ...
            if (cursor != null) {
                try {
                    if (cursor.getCount() > 0) {
                        ...
                        cursor.moveToFirst();
                        MmsLog.d(MmsApp.TXN_TAG, "duplicate, location="
