...
import com.android.mms.ui.ScheduledMessageUtils; //add JEYS-57 zhangzixiao 20131226
...
import com.android.mms.ui.BlackMessageUtils;  //add JBLW-482 zhangzixiao 20131216
import com.android.mms.data.Conversation;//add chenshu
...
//        }
    ...
    private void handleServiceStateChanged(Intent intent) {
        /// M:
        MmsLog.i(MmsApp.TXN_TAG, "Sms handleServiceStateChanged");
...
*/
        ...
        if (c != null) {
            try {
                if (c.moveToFirst()) {
                    ...
                    if(MmsConfig.isEnable7BitSmsEncoding()) {
                        SharedPreferences prefs = PreferenceManager.getDefaultSharedPreferences(MmsApp.getApplication());
                        final String encondingMode = prefs.getString(SmsPreferenceActivity.SMS_ENCODING_MODE, 
                        ...
                        if(encondingMode != null && encondingMode.equals(SmsPreferenceActivity.SETTING_ENCONDING_MODE_7BIT)) {
                            String tmpStr = MessageUtils.findSpanishChar(msgText);
                            if(tmpStr != null) {
                                //Log.e("chenshu","SmsReciverService->sendFirstQueuedMessage->msgText->"+msgText);
                                //Log.e("chenshu","SmsReciverService->sendFirstQueuedMessage->tmpStr->"+tmpStr);
                                msgText = tmpStr;
                            }
                        } else if (encondingMode.equals(SmsPreferenceActivity.SETTING_ENCONDING_MODE_AUTO)) {
                            //Log.d("chenshu","SmsReciverService->>sendFirstQueuedMessage->>Use auto mode encoding sms");
                        } else {
                            //Log.e("chenshu","SmsReciverService->>sendFirstQueuedMessage->>encondingMode is null");
                        }
                    }     
	...
	if(BlackMessageUtils.supportBlacklist()) {
	    String address = msgs[0].getDisplayOriginatingAddress();
            long blacktextId = BlackMessageUtils.getNumberInBlacklist(this, address);
	    Log.e(TAG, "handleSmsReceived, address="+address+",blacktextId="+blacktextId);
            if(blacktextId > 0) {
            	BlackMessageUtils.insertSmsToBlacklist(this, msgs, error, blacktextId);
            	return;
            }   
	}
        ...
        if (messageUri != null) {
            ...
            Conversation con = Conversation.get(this, threadId, false);
	...
	if(ScheduledMessageUtils.supportScheduledMessage(this)) {
	    numMoved += ScheduledMessageUtils.init(this);
	}
    ...
    private Uri replaceMessage(Context context, SmsMessage[] msgs, int error) {
        /// M:
        MmsLog.i(MmsApp.TXN_TAG, "Sms replaceMessage");
        SmsMessage sms = msgs[0];
...
//    private static int count = 0;
    ...
    private Uri storeMessage(Context context, SmsMessage[] msgs, int error) {
        /// M:
        MmsLog.i(MmsApp.TXN_TAG, "Sms storeMessage");
        SmsMessage sms = msgs[0];
...
//        }
        ...
        if (storeLocation.equals("Sim")) {
            ...
            ArrayList<String> messages = SmsManager.getDefault().divideMessage(smsBody);
    ...
    private String getNoServiceSimString() {
        ...
        while (noServiceIterator.hasNext()) {
            ...
            MmsLog.d(TAG, "getNoServiceSimString :: subID = " + subId);
            if ((TelephonyManagerEx.getDefault().isWifiCallingEnabled(subId))) {
                MmsLog.d(TAG, "Wifi calling registered :: subID = " + subId);
                continue;
            }
            ...
            MmsLog.d(TAG, "Added to no service string ");
        }
