...
public class RetryScheduler implements Observer {
                ...
                if ((cursor.getCount() == 1) && cursor.moveToFirst()) {
                    ...
                    if ((retryIndex < scheme.getRetryLimit()) && retry) {
                        ...
                        if (retryIndex > notifyIndex
                            ...
                            MmsLog.d(MmsApp.TXN_TAG,
                                    "to show MSG_SHOW_TRANSIENTLY_FAILED_NOTIFICATION at "
                                    + retryIndex);
                            MmsApp.getToastHandler().
                                sendEmptyMessage(MmsApp.MSG_SHOW_TRANSIENTLY_FAILED_NOTIFICATION);
                        }
                        /// @}
                        /// @}
                    } else {
			    	...
			    	//Modify, JWLW-1141, zhangzixiao, 20131118, Start
                            //MessagingNotification.notifySendFailed(mContext, true);
			    	if(android.os.SystemProperties.getBoolean("ro.sms.dual.ringtones.support", false)) {
						int simId = cursor.getInt(cursor.getColumnIndexOrThrow(PendingMessages.SUBSCRIPTION_ID));
			    		MessagingNotification.notifySendFailed(mContext, true, simId);
			    	} else {
						MessagingNotification.notifySendFailed(mContext, true);
			    	}
        ...
        if (cursor != null) {
            try {
                if (cursor.moveToFirst()) {
                    ...
                    /// M:
                    MmsLog.d(MmsApp.TXN_TAG, "Next is scheduled at"
    ...
    private boolean checkSendMmsResponseStatus(boolean isRetryDownloading,
        ...
        boolean retry = false;
        MmsLog.d(MmsApp.TXN_TAG, "isRetryDownloading:" + isRetryDownloading +
        ...
        switch(respStatus) {
            ...
            case PduHeaders.RESPONSE_STATUS_ERROR_PERMANENT_MESSAGE_NOT_FOUND:
                DownloadManager.getInstance().
                    showErrorCodeToast(R.string.service_message_not_found);
                return false;
