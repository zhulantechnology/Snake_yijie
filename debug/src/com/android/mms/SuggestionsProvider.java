...
//add BUG_ID: JSLEL-765  zhangyuwen  20140929 (start)
import com.android.mms.data.Conversation;
import com.android.mms.ui.EncryptionActivity;
//add BUG_ID: JSLEL-765  zhangyuwen  20140929 (end)
...
public class SuggestionsProvider extends android.content.ContentProvider {
    ...
    public Cursor query(Uri uri, String[] projection, String selection,
        ...
        if (null != uri && URI_MATCHER.match(uri) == SUGGEST_URI_PATH_SHORTCUT) {
            ...
            IIpSuggestionsProviderExt suggestionsProvider =
                    IpMessageUtils.getIpMessagePlugin(getContext()).getIpSuggestionsProvider();
            u = suggestionsProvider.query(selectionArgs[0]);
            if (u == null) {
    ...
    private class SuggestionsCursor implements CrossProcessCursor {
        ...
        private class Row {
            ...
            public String getSnippet() {
                ...
                int tableToUseColumn = mDatabaseCursor.getColumnIndex("table_to_use");
                long sourceId = mDatabaseCursor.getLong(sourceIdColumn);
                long tableToUse = mDatabaseCursor.getLong(tableToUseColumn);
                long threadId = getThreadId(sourceId,tableToUse);
                Conversation conv = Conversation.get(getContext(), threadId, false);
                if(EncryptionActivity.supportEncryptedMessage()) {
                    if (conv != null && conv.mEncrypted == 1) {
                        return "******";
                    }
                }
				//add BUG_ID: JSLEL-765  zhangyuwen  20140929 (end)
        ...
        private void computeRows() {
            ...
            for (int i = 0; i < count; i++) {
                ...
                if (snippet != null && snippet.trim().length() > 0 &&
                    ...
                    MmsLog.d(LOG_TAG, "computeRows snippet = "
                            + snippet + " size = " + snippet.length());
