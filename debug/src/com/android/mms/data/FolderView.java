...
/// M: @{
    public static void startQueryForDraftboxView(AsyncQueryHandler handler,
            int token , int mPostTime) {
        handler.cancelOperation(token);
        final int queryToken = token;
...
/// @}
    ...
    public static void startQueryForInboxView(AsyncQueryHandler handler,
            int token, String where, int mPostTime) {
        handler.cancelOperation(token);
        final int queryToken = token;
        final AsyncQueryHandler queryHandler = handler;
        final int postTime = mPostTime;
    ...
    private static void markFailedSmsSeen(Context context) {
        ...
        SqliteWrapper.update(context, context.getContentResolver(),
                Sms.CONTENT_URI, values, where, null);
    }
    ...
    private static void markOutboxMmsSeen(Context context) {
        ...
        SqliteWrapper.update(context, context.getContentResolver(),
                Mms.CONTENT_URI, values, where, null);
    }
    ...
    public static void startQueryForOutBoxView(AsyncQueryHandler handler,
            int token, String where) {
        handler.cancelOperation(token);
        final int queryToken = token;
        final AsyncQueryHandler queryHandler = handler;
        final String mWhere = where;
...
/// @}
    ...
    public static void startQueryForSentboxView(AsyncQueryHandler handler,
            int token, String where) {
        handler.cancelOperation(token);
        final int queryToken = token;
        final AsyncQueryHandler queryHandler = handler;
...
/// @}
    ...
    public static void startDeleteBoxMessage(AsyncQueryHandler handler,
            int token, Uri deleteuri, String where) {
    ...
    private static void fillFromCursor(Context context, FolderView fview,
        ...
        synchronized (fview) {
            ...
            /// M: ALPS02793842, no need mute handling @{
            /*
            if (fview.mBoxType == 1) {
                if (MuteCache.getMuteEntry(fview.mThreadId) == null) {
                    ...
                    fview.mIsMute = !MessageUtils.checkNeedNotifyForFolderMode(context,
                            fview.mThreadId,
