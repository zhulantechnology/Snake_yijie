...
//    };
    ...
    public String toString() {
        return String.format("{ number=%s, name=%s, nameAndNumber=%s," +
                " label=%s, person_id=%d, hash=%d method_id=%d }",
    ...
    public static String formatNameAndNumber(String name, String number, String numberE164) {
         ...
         * }
         */
        Log.d(M_TAG, "formatNameAndNumber, name=" + name + ", number=" + number);
...
//                    }
        ...
        private Contact getContactInfoForPhoneNumber(String number) {
            ...
            if (!TextUtils.isEmpty(normalizedNumber) && !TextUtils.isEmpty(matchNumber)) {
                ...
                if (cursor == null) {
                    /// M:
                    Log.d(TAG, "queryContactInfoByNumber(" + number + ") returned NULL cursor!"
                ...
                try {
                    if (cursor.moveToFirst()) {
                        ...
                        MmsLog.d(TAG,
                            ...
                            " ready to fill contact with query result. number="
                                + normalizedNumber);
                        fillPhoneTypeContact(entry, cursor);
                    }
                } finally {
        ...
        private byte[] loadAvatarData(Contact entry) {
            ...
            if ((!entry.mIsMe && entry.mPersonId == 0)
            ...
            /// M:
            MmsLog.d(TAG, "loadAvatarData(): contactUri=" + contactUri);
            InputStream avatarDataStream = Contacts.openContactPhotoInputStream(
                        mContext.getContentResolver(),
                        contactUri);
            try {
                /// M:
                MmsLog.d(TAG, "loadAvatarData(): avatarDataStream is null?="
        ...
        private Contact getContactInfoForEmailAddress(String email) {
            // / M:
            MmsLog.d(TAG, "getContactInfoForEmailAddress(): email=" + email);
                    ...
                    while (cursor.moveToNext()) {
                        ...
                        synchronized (entry) {
                            ...
                            if (!TextUtils.isEmpty(name)) {
                                entry.mName = name;
                                if (Log.isLoggable(LogTag.CONTACT, Log.DEBUG)) {
                                    log("getContactInfoForEmailAddress: name="
                ...
                if (candidates != null) {
                    ...
                    for (int i = 0; i < length; i++) {
                        ...
                        if (isNotRegularPhoneNumber) {
                            ...
                            */
                        	boolean isBrazilConfig = mContext.getResources().getBoolean(com.android.internal.R.bool.config_use_brazil_number_format);
                        	if (android.os.SystemProperties.getInt("ro.rgk_brazil_number_match", 0) == 1 || isBrazilConfig) {
                        		if (PhoneNumberUtils.compareWithAreaCode(numberOrEmail, c.mNumber)) {
                                    return c;
                                }
                        	} else {
                        		if (PhoneNumberUtils.compare(numberOrEmail, c.mNumber)) {
                    ...
                    for (int i = 0; i < length; i++) {
                        ...
                        if (isNotRegularPhoneNumber) {
                            ...
                            */
                        	boolean isBrazilConfig = mContext.getResources().getBoolean(com.android.internal.R.bool.config_use_brazil_number_format);
                        	if (android.os.SystemProperties.getInt("ro.rgk_brazil_number_match", 0) == 1 || isBrazilConfig) {
                        		if (PhoneNumberUtils.compareWithAreaCode(number, c.mNumber)) {
                                    candidates.remove(i);
                                    break;
                                }
                        	} else {
        ...
        private String getKey(String numberOrEmail, boolean isMe) {
            ...
            MmsLog.d(TAG, "getKey(" + numberOrEmail + ", " + isMe + ") return:" + key);
            return key;
        }
