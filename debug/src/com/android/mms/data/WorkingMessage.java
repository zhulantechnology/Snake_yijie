...
public class WorkingMessage implements IWorkingMessageCallback {
    ...
    private WorkingMessage(ComposeMessageActivity activity) {
        ...
        // add  BUG_ID: JWLJ-290 JWLWKK-126 zhangyuwen  20140404(start)	    
        ...
        mText = text;
        // add  BUG_ID: JWLJ-290 JWLWKK-126 zhangyuwen  20140404(end)
...
//            SlideshowEditor slideShowEditor = new SlideshowEditor(mActivity, mSlideshow);
    ...
    public synchronized Uri saveAsMms(boolean notify) {
        ...
        if (mDiscarded) {
            LogTag.debug("saveAsMms mDiscarded: true mConversation: " + mConversation +
...
//                }
    ...
    public void saveDraft(final boolean isStopping) {
        ...
        // If we have discarded the message, just bail out.
        if (mDiscarded) {
            LogTag.debug("saveDraft mDiscarded: true mConversation: " + mConversation +
...
//            loadFromUri(uri);
    ...
    public void setWorkingRecipients(List<String> numbers) {
        ...
        /// M:
        Log.d(TAG, "setWorkingRecipients: numbers=" + s);
    }
    ...
    public void dump() {
        ...
        if (mConversation != null) {
            Log.d(TAG, "mConversation: " + mConversation.toString());
        }
    }
        ...
        new Thread(new Runnable() {
            public void run() {
                try {
                    ...
                    android.util.Log.e("chenshu","asyncUpdateDraftSmsMessage:"+conv.getThreadId());
                    if (mActivity instanceof ComposeMessageActivity) {
                    if (((ComposeMessageActivity)mActivity).oldEncryption ==1 ) {
                        if (conv!=null && conv.getThreadId() > 0 && conv.getUri() != null){
                         Uri threadUri = conv.getUri().buildUpon().appendQueryParameter("isRcse", "true").build();
                            if(threadUri != null) {
		                        ContentValues ct = new ContentValues(1);
	                            ct.put("encrypted", 1);
	                            mActivity.getApplicationContext().getContentResolver().update(threadUri, ct, null, null);
