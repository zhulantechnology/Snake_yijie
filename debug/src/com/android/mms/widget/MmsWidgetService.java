...
public class MmsWidgetService extends RemoteViewsService {
    ...
    public RemoteViewsFactory onGetViewFactory(Intent intent) {
        Log.i(TAG, "onGetViewFactory intent: " + intent);
    ...
    private static class MmsFactory
        ...
        public MmsFactory(Context context, Intent intent) {
            ...
            Log.i(TAG, "MmsFactory intent: " + intent + "widget id: " + mAppWidgetId);
            // Initialize colors
        ...
        public void onDataSetChanged() {
            ...
            synchronized (sWidgetLock) {
                ...
                Log.i(TAG, "mUnreadConvCount" + mUnreadConvCount);
                onLoadComplete();
            }
        ...
        private int queryUnreadCount() {
            ...
            Log.i(TAG, "queryUnreadCount unreadCount=" + unreadCount);
            return unreadCount;
        }
            ...
            synchronized (sWidgetLock) {
				...
				if(mContext.getResources().getBoolean(R.bool.enable_mms_cs_ui)){
					remoteViews = new RemoteViews(
                        mContext.getPackageName(), R.layout.widget_conversation2);
				}else{
					remoteViews = new RemoteViews(
                        mContext.getPackageName(), R.layout.widget_conversation);
				}
                ...
                //Add, DWYBL-198, zhangyuwen, 20150410, Start
                boolean hasDraft = conv.hasDraft();
                remoteViews.setViewVisibility(R.id.draft, hasDraft ? View.VISIBLE :
                        View.GONE);
                //Add, DWYBL-198, zhangyuwen, 20150410, End
		        //Add, DWLEB-78, chenshu, 20151124, Start
		        boolean hasError = conv.hasError();
		        remoteViews.setViewVisibility(R.id.error, hasError ? View.VISIBLE :
                          View.GONE);
		        //Add, DWLEB-78, chenshu, 20151124, End
                /// M: check mute status
                ...
                */
                ...
                //Modify DSLLE-778 chenshu 20150925 start
                //original code
                /*remoteViews.setTextViewText(R.id.subject,
                            addColor(conv.getSnippet(),
                                    conv.hasUnreadMessages() ? SUBJECT_TEXT_COLOR_UNREAD :
                                        SUBJECT_TEXT_COLOR_READ));
                */
                if (conv.mEncrypted == 1 && EncryptionActivity.encryptionEnabled()) {
                    remoteViews.setTextViewText(R.id.subject,
                            addColor("******",
                                    conv.hasUnreadMessages() ? SUBJECT_TEXT_COLOR_UNREAD :
                                        SUBJECT_TEXT_COLOR_READ));
                ...
                //Modify DSLLE-778 chenshu 20150925 end
...
//                    new Intent(mContext, ConversationList.class));
        ...
        public void onUpdate(Contact updated) {
            ...
            Log.i(TAG, "onUpdate from Contact: " + updated);
            mHandler.removeCallbacks(mUpdateFromContactRunnable);
