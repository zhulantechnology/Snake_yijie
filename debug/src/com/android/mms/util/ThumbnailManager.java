...
public class ThumbnailManager extends BackgroundLoaderManager {
    ...
    private ItemLoadedFuture getThumbnail(Uri uri, boolean isVideo,
        ...
        try {
            thumbnail = DEBUG_DISABLE_CACHE ? null : mThumbnailCache.get(uri);
        } catch (Exception e) {
            thumbnail = null;
        }
        ...
        Log.i(TAG, "getThumbnail mThumbnailCache.get for uri: " + uri + " thumbnail: " +
        ...
        return new ItemLoadedFuture() {
            ...
            public void cancel(Uri uri) {
                ...
                removeThumbnail(uri);   //if the thumbnail is half loaded, force reload next time
            }
    ...
    public class ThumbnailTask implements Runnable {
        ...
        public void run() {
            ...
            mCallbackHandler.post(new Runnable() {
                ...
                public void run() {
                    ...
                    if (callbacks != null) {
                        ...
                        for (final ItemLoadedCallback<ImageLoaded> callback : asList(callbacks)) {
                            ...
                            if (!DEBUG_DISABLE_CALLBACK) {
                                ...
                                /// M:Code analyze 001,add for ALPS00338888,
                                //set imageLoad's thumbnail uri @{
                    ...
                    if (resultBitmap != null && !mUri.toString().contains(".dthumb")) {
                        mThumbnailCache.put(mUri, resultBitmap);
                        Log.i(TAG, "in callback runnable: bitmap uri: " + mUri +
        ...
        private Bitmap getBitmap(boolean isVideo) {
            ...
            if (data != null) {
                ...
                if (bitmap == null) {
                    ...
                    Log.d(TAG, "decode thumbnail successfully");
                }
        ...
        private Bitmap resizeBitmapByScale(
            ...
            /// M: fix NPE for ALPS01509039
            if (target == null) {
                throw new OutOfMemoryError("Bitmap.createBitmap return null");
            }
        ...
        private Bitmap requestDecode(final Uri uri, Options options, int targetSize) {
            if (DrmUtilsEx.checkHasDrmContent(mContext, null, uri)) {
                return null;
            }
            ...
            try {
                ...
                Log.e(TAG, "Couldn't decodeStream bitmap", e);
            }
