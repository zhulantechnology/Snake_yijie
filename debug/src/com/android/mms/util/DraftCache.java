...
public class DraftCache {
    ...
    /// M:
    public static boolean refreshDraft(Context context, long threadId) {
        Cursor cursor = SqliteWrapper.query(
                context,
                context.getContentResolver(),
                MmsSms.CONTENT_DRAFT_URI,
                DRAFT_PROJECTION, "thread_id=" + threadId, null, null);
        Log.d(TAG, "refreshDraft threadId : " + threadId);
        try {
            if (cursor != null && cursor.getCount() > 0 && cursor.moveToFirst()) {
                if (threadId == cursor.getLong(COLUMN_DRAFT_THREAD_ID)) {
                    Log.d(TAG, "DraftCache refreshDraft thread = " + threadId + " has Draft");
                    return true;
                }
            }
        } finally {
