...
public class MmsConfig {
    ...
    //modify DWYQFSQ-212 chenshu 20160708 start
    ...
    private static int mDefaultSMSMessagesPerThread = MmsApp.getApplication().getResources().getInteger(R.integer.sms_custom_size_limit);   // default value
    private static int mDefaultMMSMessagesPerThread = MmsApp.getApplication().getResources().getInteger(R.integer.mms_custom_size_limit);    // default value
    //modify DWYQFSQ-212 chenshu 20160708 end
	...
	//add DWYQFSQ-476 chengguangshuai 20160803 start
	private static List<String> tempAllQuickTexts = new ArrayList<String>();
	//add DWYQFSQ-476 chengguangshuai 20160803 end
    ...
    public static void init(Context context) {
        ...
        mContext = context;
		// modify  BUG_ID: JWLWKK-126  zhangyuwen  20150423(start)
        initSmsSignature(context);
		// modify  BUG_ID: JWLWKK-126  zhangyuwen  20150423(end)
	...
	//add DWYQFSQ-476 chengguangshuai 20160803 start
	public static List<String> getTemplatesQuicktexts() {
		tempAllQuickTexts.clear();
    	if(allQuickTexts != null){
    		for(int i = 0;i < allQuickTexts.size();i++){
    			tempAllQuickTexts.add((i+1)+")"+allQuickTexts.get(i));
    		}
    	}
		return tempAllQuickTexts;
    }
	//add DWYQFSQ-476 chengguangshuai 20160803 end
...
//    }
   ...
   /* private static void loadSimInfo(Context context) {
    ...
    public static int getSmsToMmsTextThreshold() {
		...
		//delete BWLLYBN-261 chngguangshuai 20161028 start
        /*if (limitSmsCount > 0) {
            return limitSmsCount;
        }*/
		//delete BWLLYBN-261 chengguangshuai 20161028 end
        //Add DSYYLM-52 chenshu 20160111 start
		if(MmsApp.getApplication().getResources().getBoolean(R.bool.support_custom_sms_to_mms)){
			return MmsApp.getApplication().getResources().getInteger(R.integer.sms_to_mms_text_threshold);
		}
	...
	//Add DWYQGSB-337 chengguangshuai 20170222 start
    public static boolean isSupportAsyncUpdateWallpaper() {
        return MmsApp.getApplication().getResources().getBoolean(R.bool.support_async_update_wallpaper);
    }
	
	 public static boolean isRemoveChatWallpaper() {
        return MmsApp.getApplication().getResources().getBoolean(R.bool.remove_chat_wallpaper);
    }
    //Add DWYQGSB-337 chengguangshuai 20170222  end
	
	 //Add, Mms-SmsImportExport Start
    public static boolean isSupportSmsImportExport(){
        return SystemProperties.get("ro.rgk_mms_import_export").equals("1");
    }
    //Add, Mms-SmsImportExport End
	
    //Add, Mms-SmsEncoding7Bit Start
    public static boolean isEnable7BitSmsEncoding(){
        return MmsApp.getApplication().getResources().getBoolean(R.bool.sms_encoding_lossy);//FeatureOption.RGK_SMS_ENCODING_LOSSY;
    }
    ...
    /// @}
	// modify  BUG_ID: JWLWKK-126  zhangyuwen  20150423(start)
    public static boolean isSurpportSmsSignature() {
        return MmsApp.getApplication().getResources().getBoolean(R.bool.support_sms_signature);
    }

	private static void initSmsSignature(Context context) {
        SharedPreferences sp = PreferenceManager.getDefaultSharedPreferences(context);
        if (sp.getBoolean("pref_key_first_time", true)) {
            SharedPreferences.Editor editor = sp.edit();
            editor.putBoolean("pref_key_first_time", false);
            editor.putString("pref_key_edit_signature", context.getResources().getString(R.string.default_signature));
            editor.commit();
        }
    }
	// modify  BUG_ID: JWLWKK-126  zhangyuwen  20150423(end)
